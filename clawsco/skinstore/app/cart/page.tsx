  ...
  if (items.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <p className="text-slate-500 mb-8">No cosmetics added yet.</p>
        <Link href="/store" className="text-white px-8 py-3 rounded-xl font-medium inline-block bg-rose-600 hover:bg-rose-700 transition-colors">
          Browse Skins
        </Link>
      </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-4">
          {items.map(item => (
            <div key={item.id} className="rounded-2xl p-6 border border-slate-200 bg-white flex flex-col sm:flex-row items-center gap-6 shadow-sm">
              <div className="w-20 h-20 rounded-xl flex items-center justify-center text-3xl bg-rose-50">
                {item.emoji}
              </div>
              <div className="flex-1 text-center sm:text-left">
                <h3 className="font-bold text-lg mb-1 font-primary text-slate-900">{item.name}</h3>
                <p className="font-semibold text-rose-600">${item.price.toFixed(2)}</p>
              </div>
        ...
        <div>
          <div className="rounded-2xl p-6 border border-slate-200 bg-white sticky top-24 shadow-sm">
            ...
            <button className="w-full text-white py-4 rounded-xl font-bold transition-all active:scale-95 bg-rose-600 hover:bg-rose-700">
              Checkout
            </button>
          </div>
        </div>
